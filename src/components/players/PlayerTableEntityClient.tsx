...
import { Card, CardBody, CardContext } from '@/components/ui/card'
import {
    SmartCardFooterSync,
    SmartCardFooterContentSync,
} from '@/components/ui/SmartTabsSync'
...
const PlayerTableEntityContent = ({
    ...
    const { isInitializing } = useSimpleEntityInit(
        seasons,
        initialSeason,
        initialSplit,
        initialTournament
    ...
    const [activeStatsView, setActiveStatsView] = useState<string | null>(
        'Players'
    )
    ...
    const [fallbackTeamData, setFallbackTeamData] = useState<{
        name: string
        overviewPage?: string
        image?: string
    } | null>(null)
    ...
    const { teamData: tournamentTeamData } = useTournamentPlayerTeam(
        selectedTournamentId,
        playerName
    )
    ...
    useEffect(() => {
        ...
        const fetchTeamImage = async () => {
            ...
            try {
                const imageResult = await getTeamImage(
                    currentTeamData.image!.replace('.png', '.webp')
                )
    ...
    useEffect(() => {
        ...
        const fetchTournamentName = async () => {
            try {
                const response = await getTournamentPlayerStats(
    ...
    useEffect(() => {
        ...
        const fetchTeamData = async () => {
            try {
                console.log(
                    `🔄 [PLAYER COMPONENT] Fetching complete team data for: ${playerData.team}`
                )
                ...
                if (teamResult.data) {
                    ...
                    console.log(
                        `✅ [PLAYER COMPONENT] Fetched team data - overviewPage: ${
                            teamResult.data.overviewPage || 'N/A'
                        }`
                    )
                } else {
                    console.log(
                        `⚠️ [PLAYER COMPONENT] No team data found for: ${playerData.team}`
                    )
            ...
            <div className="block md:hidden  ">
                {playerData && (
                    <PlayerDescription
                        ...
                        teamImage={currentTeamImage || undefined}
                        ...
                        <div className="space-y-4">
                            ...
                            {selectedTournamentId ? (
                                ...
                                <div className="p-4 bg-gray-700 rounded-lg">
                                    ...
                                    <p>Select a tournament to see matches</p>
                                </div>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="space-y-4">
                        ...
                        {playerData ? (
                            <div className="grid gap-6">
                                ...
                                {(playerData.twitter ||
                                    ...
                                    <div className="bg-card rounded-lg p-6 border border-border">
                                        ...
                                        <div className="flex flex-wrap gap-4">
                                            {playerData.twitter && (
                                                <a
                                                    href={`https://twitter.com/${playerData.twitter}`}
                                                    target="_blank"
                                            ...
                                            {playerData.instagram && (
                                                <a
                                                    href={`https://instagram.com/${playerData.instagram}`}
                                                    target="_blank"
                                                    rel="noopener noreferrer"
                                ...
                                {/* Favorite Champions */}
                                {playerData.favChamps &&
                                    playerData.favChamps.length > 0 && (
                                        <div className="bg-card rounded-lg p-6 border border-border">
                                            <h2 className="text-xl font-semibold mb-4">
                                                Favorite Champions
                                            </h2>
                                            <div className="flex flex-wrap gap-2">
                                                {playerData.favChamps.map(
                                                    (
                                                        champ: string,
                                                        index: number
                                                    ) => (
...
export const PlayerTableEntityClient = ({
    ...
    const {
        ...
        loading,
        error,
    } = usePlayerTableEntityData(playerName)
